...
datasets = {x : dataset.CustomDataset(path = configer.data_path, mode = x, transform=data_transform[x]) for x in ['train', 'val']}
...
dataloders = {x : DataLoader(datasets[x], batch_size=configer.batch_size, shuffle=True, drop_last=True) for x in ['train', 'val']}
...
model_ft = utils_file.train_model(net, criterion, optimizer,  dataloders, dataset_sizes, lr_schedule, configer.num_epochs)
