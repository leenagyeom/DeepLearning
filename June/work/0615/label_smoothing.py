...
class LabelSoothingCrossEntropy(nn.Module):
    def __init__(self):
        super(LabelSoothingCrossEntropy, self).__init__()
...
criterion = LabelSoothingCrossEntropy()
...
def loss_acc_visuaize(history, optim, path):
    ...
    plt.savefig(str(path) + 'loss_acc_label.png')
...
def grad_visualize(history, optim, path, ylimit=10):
    ...
    plt.savefig(str(path) + 'grad_norms_label.png')
