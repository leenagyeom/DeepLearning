...
def data_count(main_path='../../14_pretrained/dataset/YogaPoses/'):
    counts = {}
...
def data_split(main_path='../../14_pretrained/dataset/YogaPoses/', split_percentage=0.2):
    train_dict = {}
    valid_dict = {}
...
def loss_acc_visuaize(history, optim, path):
    ...
    plt.savefig(str(path) + 'loss_acc_mixup.png')
...
def grad_visualize(history, optim, path, ylimit=10):
    ...
    plt.savefig(str(path) + 'grad_norms_mixup.png')
...
def mixupFunction(x, y, alpha = 1.0, train_on_gpu = True):
...
for epoch in range(n_epochs):
    ...
    for ii, (data, target) in enumerate(train_loader):
        ...
        data, targetA, targetB, lam = mixupFunction(data, target)
        ...
        loss = mixupCriterion(criterion, output, targetA, targetB, lam)
        loss.backward()
